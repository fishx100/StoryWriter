...
export default async function CharactersPage({ params }: CharactersPageProps) {
  ...
  async function handleDeleteCharacter() {
    ...
    setDeleting(true);
    try {
      await fetchJson<void>(`/api/characters/${deleteTarget.id}`, { method: 'DELETE' });
  ...
  async function handleReorder(order: string[]) {
    try {
      await fetchJson<void>(`/api/works/${workId}/characters/reorder`, { method: 'POST', body: JSON.stringify({ order }) });
      setCharacters(current => {
        const map = new Map(current.map(character => [character.id, character]));
        return order.map(id => map.get(id)).filter((character): character is Character => character !== undefined);
      });
    } catch { setError('Failed to reorder characters.'); }
  }
      ...
      <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,_rgba(251,191,36,0.08),_transparent_30%),linear-gradient(180deg,#020617_0%,#020617_45%,#07111f_100%)] px-4 py-6 text-slate-100 sm:px-6 lg:px-8">
        <div className="mx-auto flex max-w-7xl flex-col gap-6">
          ...
          <div className="rounded-[2rem] border border-slate-200/10 bg-slate-950/80 p-5 shadow-2xl shadow-black/20 backdrop-blur sm:p-6">
            ...
            {loading ? <p>Loading...</p> : <CharacterList workId={workId} characters={characters} onRequestDelete={setDeleteTarget} onReorder={handleReorder} />}
