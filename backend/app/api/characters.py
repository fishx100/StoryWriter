...
from app.infrastructure.models import CharacterModel, WorkModel
...
def list_characters(work_id: str, db: Session = Depends(get_db), current_user: AuthenticatedUser = Depends(get_current_user)) -> list[CharacterRead]:
...
def create_character(work_id: str, payload: CharacterCreate, db: Session = Depends(get_db), current_user: AuthenticatedUser = Depends(get_current_user)) -> CharacterRead:
    work = db.get(WorkModel, work_id)
    if work is None or work.user_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='Work not found')
...
def update_character(work_id: str, character_id: str, payload: CharacterCreate, db: Session = Depends(get_db), current_user: AuthenticatedUser = Depends(get_current_user)) -> CharacterRead:
...
def delete_character(work_id: str, character_id: str, db: Session = Depends(get_db), current_user: AuthenticatedUser = Depends(get_current_user)) -> None:
    model = db.get(CharacterModel, character_id)
    if model is None or model.work_id != work_id:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='Character not found')
    work = db.get(WorkModel, work_id)
...
def reorder_characters(work_id: str, payload: CharacterReorder, db: Session = Depends(get_db), current_user: AuthenticatedUser = Depends(get_current_user)) -> None:
    work = db.get(WorkModel, work_id)
    if work is None or work.user_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='Work not found')
